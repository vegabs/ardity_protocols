...
public class SerialControllerCustomDelimiter : MonoBehaviour
{
    ...
    private void Start()
    {
        dropdownPort.onValueChanged.AddListener(delegate { changePortName(dropdownPort); });
    }

    void changePortName(Dropdown mydrop)
    {
        portName = mydrop.options[dropdownPort.value].text;
    }
    ...
    void Update()
    {
        ...
        if (message == null)
        {
            // Debug.LogError("El binario llegó nulo :(");
            return;
        }
